...
export default function StudentNotificationsPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("student-notifications-inbox-" + Math.random().toString(36).substring(7))
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  // Helper to resolve categories to icons & styling
  const getCategoryStyles = (type: string) => {
    const t = type.toUpperCase();
    if (t.includes("ENROLLMENT")) {
      return {
        icon: GraduationCap,
    ...
    if (t.includes("EXAM") || t.includes("RESULT")) {
      return {
        icon: ListTodo,
  ...
  const getRedirectUrl = (entityType: string | null, entityId: string | null) => {
    ...
    if (type === "enrollments") return "/student";
    if (type === "payments") return "/student/payments";
    if (type === "batch_contents") return `/student/exams`; // redirect to generic batch exams/materials
    if (type === "exams") return `/student/exams`;
    if (type === "exam_result") return `/student/results`;
    return null;
  };
  ...
  return (
    <div className="space-y-8 text-xs font-bold text-primary">
      ...
      <DashboardPageHeader
        ...
        description="Stay updated with notices, batch schedules, new materials, and verified exam results."
        actions={
          unreadCount > 0 && (
            <button
      ...
      <div className="bg-white p-4 rounded-2xl border border-border/40 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        ...
        <div className="flex items-center gap-2.5 w-full sm:w-auto">
          ...
          >
            ...
            <option value="exam/result">Exams & Results</option>
          </select>
        </div>
      </div>
      ...
      <div className="bg-white border border-border/40 rounded-2xl shadow-sm overflow-hidden divide-y divide-border/20">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <span className="text-muted font-semibold">Loading inbox notifications...</span>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center px-4">
            ...
            <h4 className="text-sm font-bold text-primary">Your notification inbox is empty</h4>
            <p className="text-xs text-muted font-medium mt-1 max-w-xs leading-relaxed">
              {filter === "unread"
                ? "No unread alerts found. You are completely caught up!"
                : "No announcements, results, or invoice updates scheduled."}
            </p>
          </div>
        )}
      </div>
